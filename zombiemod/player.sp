...
public OnClientPutInServer(client)
{
	...
	{
		// TODO: Add to convars.sp
		static Handle:alliesSniperLimit, Handle:alliesMgLimit, Handle:alliesRocketLimit;
		if (!alliesSniperLimit)
		{
			alliesSniperLimit = FindConVar("mp_limit_allies_sniper");
		}
		if (!alliesMgLimit)
		{
			alliesMgLimit = FindConVar("mp_limit_allies_mg");
		}
		if (!alliesRocketLimit)
		{
		...
		else
		{
			switch (GetClientTeam(client))
			{
				...
				case Team_Axis:
				{
					...
					g_ClientInfo[client][ClientInfo_SniperCrit] = false;

					RemoveWeapons(client);
					GivePlayerItem(client, "weapon_spade");

					PlaySoundFromPlayer(client, g_szSounds[Sound_ZombieSpawn]);
					SetPlayerModel(client, IsClientAdmin(client) ? Model_Zombie_Admin : Model_Zombie_Default);
...
public Action:OnJoinClass(client, &playerClass)
{
	if (g_bModActive)
	{
		if (GetClientTeam(client) == Team_Allies)
		{
			...
			{
				if (IsPlayerClassRestricted(playerClass))
				{
					ZM_PrintToChat(client, "This class can now only be selected by VIP members!");
					return Plugin_Handled;
				}
				else if (playerClass == PlayerClass_Random)
				{
					return Plugin_Handled;
				}
			}
...
public Action:OnWeaponCanUse(client, weapon)
{
	if (g_bModActive && g_ClientInfo[client][ClientInfo_WeaponCanUse])
	{
		...
		if (clientTeam == Team_Allies && !IsClientVIP(client))
		{
			static const String:vipWeapons[][] = 
			{
				"spring",
				"k98_scoped",
				"bazooka",
			...
			{
				if (StrEqual(className[7], vipWeapons[i])) // Skip the first 7 characters in className to avoid comparing the "weapon_" prefix.
				{
					return Plugin_Handled;
				}
			}
	...
	if (g_bModActive)
	{
		if (attacker >= 1 && attacker < MaxClients && GetClientTeam(client) == Team_Axis)
		{
			...
			if (activeWeapon != -1)
			{
				...
				if (g_ClientInfo[client][ClientInfo_SniperCrit] && (StrEqual(className[7], "spring") || StrEqual(className[7], "k98_scoped")))
				{
					return Plugin_Handled;
				}
			}*/
