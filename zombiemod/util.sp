...
bool:DoSelectZombie(Handle:array)
{
	new arraySize = GetArraySize(array);

	if (arraySize)
	{
		g_iZombie = GetArrayCell(array, arraySize >= 2 ? GetURandomInt() % (arraySize - 1) : 0);
		return true;
	}
...
SelectZombie()
{
	new Handle:clientArray = CreateArray(), Handle:prioritizedArray = CreateArray();

	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsClientSourceTV(i) && GetClientTeam(i) > Team_Spectator)
		{
			PushArrayCell(i == g_iZombie || IsClientVIP(i) ? prioritizedArray : clientArray, i);
		}
	}

	if (!DoSelectZombie(clientArray) && !DoSelectZombie(prioritizedArray))
	{
...
IsClientVIP(client)
{
	...
	return adminId != INVALID_ADMIN_ID && GetAdminFlag(adminId, Admin_Custom1);
}

bool:IsMachineGunnerClassFull()
{
	new numMachinegunners = 0;

	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && GetClientTeam(i) == Team_Allies && !IsClientVIP(i) && GetDesiredPlayerClass(i) == PlayerClass_Machinegunner)
		{
			numMachinegunners++;
		}
	}

	// TODO: add convar, dont forget to add this to the fake convar at player connect.
	return numMachinegunners >= 1;
}

bool:IsPlayerClassRestricted(playerClass)
{
